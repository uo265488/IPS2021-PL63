...
public class RevisorView extends JDialog {
	...
	if (btEnviar == null) {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (getTxAutor().getText().strip().length() == 0
			    ...
			    int numeroRevision = controller.numeroRevisiones(idArt, idRev);
			    controller.actualizarRevision(getTxAutor().getText(), getTxEditor().getText(),
				    (String) getChDecision().getSelectedItem(), true, idRev, idArt, numeroRevision);
	...
	if (btGuardarCambios == null) {
		...
		public void actionPerformed(ActionEvent e) {
			...
			int numeroRevision = controller.numeroRevisiones(idArt, idRev);
			controller.actualizarRevision(getTxAutor().getText(), getTxEditor().getText(),
				(String) getChDecision().getSelectedItem(), false, idRev, idArt, numeroRevision);
			limpiar();
			getBtEnviar().setEnabled(false);
			getBtGuardarCambios().setEnabled(false);
		    }
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (idArt == null)
			    JOptionPane.showMessageDialog(null,
				    ...
				    JOptionPane.ERROR_MESSAGE);
			else if (controller.numeroRevisiones(idArt, idRev) != 2)
			    ...
			    RevisionEntity revision = controller.getRevisionAnterior(idArt, idRev);
