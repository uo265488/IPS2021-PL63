...
public class EditorDebateView extends JDialog {
	...
	setTitle("Editor. Debate de un articulo");
    }
	...
	if (btnEnviar == null) {
		...
		public void actionPerformed(ActionEvent e) {
		    ...
		    String mensaje = "Editor [" + fecha.format(isoFecha) + " - " + fecha.format(isoHora) + "] - "
			    + getTxtMensajeEnvio().getText();
		    if (mensaje.length() > 140) {
			JOptionPane.showMessageDialog(null, "Demasiado largo", "Error de tamaño",
				JOptionPane.ERROR_MESSAGE);
		    } else if (getTxtMensajeEnvio().getText().trim().equals("")) {
			JOptionPane.showMessageDialog(null, "Por favor introduce un mensaje", "Error de tamaño",
				JOptionPane.ERROR_MESSAGE);
		    } else {
