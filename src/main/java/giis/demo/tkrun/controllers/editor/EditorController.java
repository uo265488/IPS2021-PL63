...
public class EditorController {
	...
	int numeroRevision = revisionModel.numeroRevisiones(revisor.getId(), articulo.getIdArticulo()).size() + 1;

	cambiarEstadoRevisorNoDisponible(revisor);

	generarRevision(revisor, articulo, fecha, numeroRevision);
     ...
     * @return
     */
    private void generarRevision(RevisorEntity revisor, ArticuloEntity articulo, String fecha, int numeroRevision) {

	revisionModel.add(DtoMapper.toRevisionDto(revisor, articulo, fecha, RevisionEntity.PENDIENTE, numeroRevision));
    }
